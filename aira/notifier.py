...
def post_file(client, channel, path, thread_ts=None, title="Aira output"):
    try:
        if thread_ts:
...
def approve_blocks(question):
    action_id = "aira_approve_" + uuid.uuid4().hex[:10]
    return action_id, [
        {
            "type": "section",
            "text": {"type": "mrkdwn", "text": f":warning: *Aira needs approval*\n{question}"},
        },
        {
            "type": "actions",
            "elements": [
                {"type": "button", "text": {"type": "plain_text", "text": "Approve"}, "style": "danger", "action_id": "aira_approve", "value": action_id},
                {"type": "button", "text": {"type": "plain_text", "text": "Deny"}, "action_id": "aira_deny", "value": action_id},
            ],
        },
    ]
...
def ack_blocks(digest_id):
    return [
        {
            "type": "section",
            "text": {"type": "mrkdwn", "text": "⬆️ *Aira email digest* — press *Got it* once you have read it."},
        },
        {
            "type": "actions",
            "elements": [{"type": "button", "text": {"type": "plain_text", "text": "Got it ✅"}, "action_id": "aira_ack", "value": digest_id}],
        },
    ]
